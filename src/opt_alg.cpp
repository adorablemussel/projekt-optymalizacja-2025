...
solution fib(matrix(*ff)(matrix, matrix, matrix), double a, double b, double epsilon, matrix ud1, matrix ud2)
{
	...
	{
    solution Xopt;
    // // Oblicz ciąg Fibonacciego
    // std::vector<double> phi = {1, 1};
    // while (phi.back() <= (b - a) / epsilon)
    //     phi.push_back(phi[phi.size() - 1] + phi[phi.size() - 2]);
    // int k = phi.size() - 1;

    // // 2. Wyznaczamy punkty startowe
    // double c = b - (phi[k - 1] / phi[k]) * (b - a);
    // double d = a + b - c;

    // // 3. Pętla optymalizacyjna
    // for (int i = 0; i < k - 2; ++i)
    // {
    //     if (m2d(ff(matrix(1, 1, c), ud1, ud2)) < m2d(ff(matrix(1, 1, d), ud1, ud2)))
    //         b = d;
    //     else
    //         a = c;
